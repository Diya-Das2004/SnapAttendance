...
# face image--> face detector(dlib)--> shape predictor(sp)-landmarks--> ReSNet(face rec)-embedding(128D)
@st.cache_resource
def load_dlib_models():
    detector= dlib.get_frontal_face_detector() #boundary box asar jonno scan er
...
def predict_attendence(class_image_np):
    ...
    for encoding in encodings:
        ...
        best_match_score= np.linalg.norm(student_embedding - encoding_np)
