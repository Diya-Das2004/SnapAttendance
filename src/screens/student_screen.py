...
def student_screen():
    ...
    if st.session_state.get("show_registration", False):
        with st.container(border= True):
            ...
            new_name= st.text_input("Enter your name",placeholder= "E.g. Diya Das")
            st.subheader("Optional: voice enrollment")
            st.info("Enroll your voice only attendence")
            audio_data= None
            try:
                audio_data= st.audio_input('Record a short phrase like I am present, My name is Diya')
            except Exception:
                st.error('Audio_data Failed')
